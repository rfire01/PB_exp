...
async function exportExperimentsFromID(EXPID) {
    const Json2csvParser = require('json2csv').Parser;
    const fastcsv = require("fast-csv");
    const fs = require("fs");
    const excel = require('exceljs');
    ...
    let experiments = await db.executeTrustedQuery('SELECT * FROM EXPERIMMENTS WHERE TOKEN IS NOT NULL AND EXP_ID>' + EXPID);
    ...
    workbook.xlsx.writeFile("experimentsByDate.xlsx")
        .then(function () {
            console.log("file saved! from ");
          });

}


async function exportLimitedToExcel(EXPID) {
    const Json2csvParser = require('json2csv').Parser;
    const fastcsv = require("fast-csv");
    const fs = require("fs");
    const excel = require('exceljs');
    ...
    let arranged_items = await db.executeTrustedQuery('select * from ARRANGED_ITEMS');
    let blacklist = await db.executeTrustedQuery('select * from BLACKLIST');
    let exp_items = await db.executeTrustedQuery('select * from EXP_ITEMS WHERE EXP_ID>' + EXPID);
    let items = await db.executeTrustedQuery('select * from ITEMS');
    let participants = await db.executeTrustedQuery('select * from PARTICIPANTS');
    ...
    workbook.xlsx.writeFile("EXP-limited.xlsx")
        .then(function () {
            console.log("file limited saved!");
        });
